...
//
//    public LinkedList<LinkedList<RoadTrip>> makeAMatch(TripRequest request, int numberOfTripsToOffer) {  //By departure or arrival
...
//    }
//
//    private LinkedList<LinkedList<RoadTrip>> collectMatchingTripsByDepartureTime(TripRequest request, int numberOfTripsToOffer) {
//
//        int index;
//        LinkedList<RoadTrip> combinedTrip = new LinkedList<>();
//        LinkedList<LinkedList<RoadTrip>> res = new LinkedList<>();
//
//        for (Map.Entry<Integer, TripSuggest> entry : data.getPlannedTrips().getTrips().entrySet()) {
...
//    }
//
//    private void buildMatchingTripsByDeparture(LinkedList<RoadTrip> ride, TripRequest request, LinkedList<LinkedList<RoadTrip>> res, int numberOfTripsToOffer) {
//
...
//        } else {
//            LinkedList<RoadTrip> matchingRides = findTripsForNextStations(ride);
//            for (RoadTrip matchedRide : matchingRides) {
//                LinkedList<RoadTrip> newRide = copyLinkedList(ride);
//                if (matchedRide.getTrip().getSerialNumber() == ride.getLast().getTrip().getSerialNumber()) {
//                    newRide.getLast().setEndStationInRoute(matchedRide.getRoute().getLast(), findClosestDayFromAbove(matchedRide.getTrip(), matchedRide.getTrip().getSchedule().recurrencesToInteger(), matchedRide.getFirstStation(), newRide.getLast().getLastStation().getDay(), newRide.getLast().getLastStation().getHour(), newRide.getLast().getLastStation().getMinutes()));
//                    buildMatchingTripsByDeparture(copyLinkedList(newRide), request, res, numberOfTripsToOffer);
//                    newRide.getLast().getRoute().removeLast();
//                } else {
//                    newRide.add(new RoadTrip(matchedRide, findClosestDayFromAbove(matchedRide.getTrip(), matchedRide.getTrip().getSchedule().recurrencesToInteger(), matchedRide.getFirstStation(), newRide.getLast().getLastStation().getDay(), newRide.getLast().getLastStation().getHour(), newRide.getLast().getLastStation().getMinutes())));
//                    buildMatchingTripsByDeparture(copyLinkedList(newRide), request, res, numberOfTripsToOffer);
//                    newRide.removeLast();
//                }
//            }
...
//    }
//
//    private LinkedList<RoadTrip> copyLinkedList(LinkedList<RoadTrip> ride) {
//        LinkedList newList = new LinkedList<RoadTrip>();
//        for (RoadTrip matchedRide : ride) {
//            newList.add(new RoadTrip(matchedRide));
//        }
//        return newList;
//    }
//
//
//    public LinkedList<RoadTrip> findTripsForNextStations(LinkedList<RoadTrip> ride) {
//
//        LinkedList<RoadTrip> matchingRides = new LinkedList<>();
...
//                    matchingRides.add(new RoadTrip(entry.getValue(), entry.getValue().getRide()[index], entry.getValue().getRide()[index + 1], findClosestDayFromAbove(entry.getValue(), entry.getValue().getRecurrencesType().getTripScheduleTypeString(), entry.getValue().getRide()[index], ride.getLast().getLastStation().getDay(), ride.getLast().getLastStation().getHour(), ride.getLast().getLastStation().getMinutes())));
//            }
//        }
//        return matchingRides;
//    }
//
//    private boolean isTheStationExist(LinkedList<RoadTrip> ride, Station station) {
//        for (RoadTrip mr : ride) {
...
////~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~arrival~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
//
//    private LinkedList<LinkedList<RoadTrip>> collectMatchingTripsByArrivalTime(TripRequest request, int numberOfTripsToOffer) {
//
//        LinkedList<RoadTrip> combinedTrip = new LinkedList<>();
//        LinkedList<LinkedList<RoadTrip>> res = new LinkedList<>(); // save all the optional result of combinedTrips
//
//        for (Map.Entry<Integer, TripSuggest> entry : data.getPlannedTrips().getTrips().entrySet()) {
...
//                    combinedTrip.add(new RoadTrip(entry.getValue(), entry.getValue().getRide()[index - 1], entry.getValue().getRide()[index], findClosestDayFromBelow(entry.getValue(), entry.getValue().getRecurrencesType().getValue(), entry.getValue().getRide()[index], request.getArrivalTime().getDay(), request.getArrivalTime().getHours(), request.getArrivalTime().getMinutes())));
...
//    }
//
//    private void buildMatchingTripsByArrival(LinkedList<RoadTrip> ride, TripRequest request, LinkedList<LinkedList<RoadTrip>> res, int numberOfTripsToOffer) {
//
//        if (res.size() == numberOfTripsToOffer)
//            return;
//
...
//            res.add(ride);
//        } else {
//            LinkedList<RoadTrip> matchingRides = findTripsForPrevStations(ride); //gating list of optional Plaid drive
//            for (RoadTrip matchedRide : matchingRides) {
//                LinkedList<RoadTrip> newRide = copyLinkedList(ride);
...
//                } else {
//                    newRide.addFirst(new RoadTrip(matchedRide, findClosestDayFromBelow(matchedRide.getTrip(), matchedRide.getTrip().getSchedule().recurrencesToInteger(), matchedRide.getRoute().getLast(), newRide.getFirst().getRoute().getFirst().getDay(), newRide.getFirst().getRoute().getFirst().getHour(), newRide.getFirst().getRoute().getFirst().getMinutes())));
...
//    }
//
//    private LinkedList<RoadTrip> findTripsForPrevStations(LinkedList<RoadTrip> ride) {
//
//        LinkedList<RoadTrip> matchingRides = new LinkedList<>();
//        Station current = ride.getFirst().getRoute().getFirst();
...
//                        isTheStationExist(ride, entry.getValue().getRide()[index - 1]))
//                    matchingRides.add(new RoadTrip(entry.getValue(), entry.getValue().getRide()[index - 1], entry.getValue().getRide()[index], findClosestDayFromBelow(entry.getValue(), entry.getValue().getRecurrencesType().getValue(), entry.getValue().getRide()[index], ride.getFirst().getRoute().getFirst().getDay(), ride.getFirst().getRoute().getFirst().getHour(), ride.getFirst().getRoute().getFirst().getMinutes())));
//            }
//
//        }
//        return matchingRides;
